...
module.exports = {
  config: {
    ...
    longDescription: {
      ...
      fr: "traduire un texte avec une langue aux choix"
    },
    category: "𝗨𝗦𝗘𝗙𝗨𝗟",
    guide: {
      ...
      fr: "   {pn} <texte>: Traduire le texte dans la langue de votre boîte de discussion ou la langue par défaut du bot"
        + "\n   {pn} <texte> -> <ISO 639-1>: Traduire le texte dans la langue souhaitée"
        + "\n   ou vous pouvez répondre à un message pour traduire le contenu de ce message"
        + "\n   Exemple :"
        + "\n    {pn} bonjour -> vi"
        + "\n   {pn} -r [on | off]: Activer ou désactiver le mode de traduction automatique lorsqu'une personne réagit au message"
        + "\n   {pn} -r set <emoji>: Définir l'emoji pour traduire le message dans votre groupe de discussion"
    }
  },
  ...
  langs: {
    ...
    fr: {
      translateTo: "🌐 Traduire de %1 à %2",
      invalidArgument: "❌ Argument non valide, veuillez choisir on ou off",
      turnOnTransWhenReaction: `✅ Activer la traduction des messages lors d'une réaction, essayez de réagir avec \"${defaultEmojiTranslate}\" à n'importe quel message pour le traduire (ne prend pas en charge les messages de bot)\n Ne traduit que les messages après avoir activé cette fonctionnalité`,
      turnOffTransWhenReaction: "✅ Désactiver la traduction des messages lors d'une réaction",
      inputEmoji: "🌀 Veuillez réagir à ce message pour définir cet emoji comme emoji pour traduire le message",
      emojiSet: "✅ L'emoji pour traduire le message est défini sur %1"
    }
  },
